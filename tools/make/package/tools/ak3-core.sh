...
flash_boot() {
  ...
  if [ ! -f boot-new.img ]; then
    ...
    abort "  - Error! New image larger than target partition. Aborting...";
  fi;
...
flash_generic() {
  ...
  if [ "$img" -a ! -f ${1}_flashed ]; then
    for path in /dev/block/by-name /dev/block/bootdevice/by-name /dev/block/mapper; do
...
setup_ak() {
  ...
  if [ ! "$no_block_display" ]; then
    ui_print "  - Installing kernel to $block";
  fi;
}
###
